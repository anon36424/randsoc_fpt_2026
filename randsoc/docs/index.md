...
`CONFIG` (config yaml), `PART`, `SEED`, `VIVADO` (vivado path), `JOBS`, `N` (design count), `SYNTH`, `TIMEOUT`.
...
| `make vivado-test` | Build `N` designs and run them through local Vivado up to BD validation. Add `SYNTH=1` for full synthesis (passes only if `synth.dcp` is produced); raise `TIMEOUT` and lower `JOBS` for synthesis. |
